...
def lstm_model_onelayer(input_dim = 1,out_dim = 1,timesteps = 4,unit1=100):
                               # expected input data shape: (batch_size, timesteps, data_dim)
    model = Sequential()
    model.add(LSTM(unit1,input_shape = (timesteps, input_dim)))  # returns a sequence of vectors of dimension 32
...
model = lstm_model_onelayer(input_dim=1,out_dim=1)
...
pd.DataFrame(p).to_csv('Prediction_Results_onelayer.csv')
...
plt.title('LSTM_onelayer')
plt.savefig('LSTM_onelayers.jpg')
plt.show()